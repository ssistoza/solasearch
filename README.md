...
A personal, self-hosted Kagi search frontend. Full SSR on cold loads, instant
SPA navigation with skeleton loaders, a 24-hour result cache, and DDG-style
`!bang` support — wrapped in a fast, keyboard-friendly UI.
...
├── src/
│   ├── routes/
│   │   ├── __root.tsx           # Root document (head, styles, scripts)
│   │   ├── index.tsx            # Homepage: prompt-style search box, bang chips
│   │   ├── search.tsx           # Search layout: header form, tabs, validation
│   │   └── search/
│   │       ├── index.tsx        # Web results
│   │       ├── images.tsx       # Image results
│   │       ├── videos.tsx       # Video results
│   │       └── news.tsx         # News results
│   ├── components/
│   │   ├── pagination.tsx       # Page navigation
│   │   └── skeletons.tsx        # Loading skeletons (shared pending UI)
│   ├── utils/
│   │   ├── kagi.ts              # searchKagi server function + types
│   │   ├── bangs.ts             # Bang registry + resolver
│   │   ├── auth.ts              # Device token validation
│   │   └── cache-control.ts     # Request middleware for Cache-Control headers
│   ├── start.ts                 # Global Start instance (middleware wiring)
│   ├── router.tsx               # Router setup + caching defaults
│   ├── routeTree.gen.ts         # Auto-generated by TanStack Router — do not edit
│   └── styles.css               # Tailwind entry, theme tokens, font import
├── .env.schema                  # Varlock schema (committed, no secrets)
└── vite.config.ts               # Vite config (Start, Tailwind, Varlock plugins)
```
